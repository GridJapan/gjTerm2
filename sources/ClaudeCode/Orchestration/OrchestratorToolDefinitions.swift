...
extension OrchestratorCommand {
    ...
    static var allToolDefinitions: [ToolDefinition] { [
        ...
        ToolDefinition(
            ...
            description: "List all active workgroups and their member sessions. Each session reports its role within the workgroup, current status (idle/working/waiting), status_source, and kind (claude-code/shell/tui/other). status_source says how much to trust `status`: \u{201C}reported\u{201D} means the program announces its own status (accurate, and state watchers fire on exact transitions); \u{201C}inferred\u{201D} means gjTerm2 is guessing from indicators, so `status` is unreliable (e.g. a program waiting at a splash screen looks \u{201C}idle\u{201D}). screen_last_changed says how long ago the session's screen last changed; use it to tell stale output from fresh activity. provenance (on workgroups and sessions) says who created them, e.g. a workgroup entered by a trigger in some session, or a session created by an agent in another chat; a workgroup whose provenance traces to a different chat belongs to that chat's work, so don't drive it unless asked. Standalone sessions appear as single-session workgroups with a synthetic ID prefixed \u{201C}session:\u{201D}.",
        ...
        ToolDefinition(
            ...
            inputSchema: object([
                ...
                ("notify_user", boolean("Set true when the user asked to be told/alerted when this happens. gjTerm2 sends a push notification to their iPhone automatically when the watch fires; you do not need to call notify yourself.")),
            ] : []), required: ["session_guid"])),
