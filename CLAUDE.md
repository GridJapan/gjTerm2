...
- **Config dotdir `~/.config/iterm2` / `~/.iterm2` — BLOCKED on the shell-integration submodule.** Central derivation is `baseName ?: @"iterm2"` in `NSFileManager+iTerm.m`, but the shell-integration scripts that hardcode `.iterm2` live in `submodules/iTerm2-shell-integration` (a git submodule — do not modify without express permission). Changing only the app side would desync `~/.iterm2/shell-integration` (`Conductor.swift`, `iTermShellIntegrationWindowController.m`) from the scripts. Do the app + submodule together, or leave both. (This means gjTerm2 currently shares `~/.config/iterm2` with stock iTerm2 — incl. the `sockets/secrets` socket.)
